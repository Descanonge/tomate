...
:class:`Variable<variable_base.Variable>` manage the data for one variable.
...
:class:`VariablesMasked<var_types.variable_masked.VariableMasked>`
...
Tomate will try to guess the dimensions from filegroup if possible, if not it
will use all the dimensions available.

.. warning::

   This guess can be very wrong, and cause silent bugs. Mainly because
   the dimensions found *in-file* are not necessary those of the variable
   (if the dimensions shared for example).
   Always check its not doing anything stupid or specify by hand the dimensions.
...
.. note::
   ...
   To quickly create variables objects, one can use `DataBase.create_variables`
   to create all variable objects from variables in the available scope in one
   go.
...
be accessed conveniently using :func:`Variable.view
<variable_base.Variable.view>`
or :func:`Variable.view_by_value<variable_base.Variable.view_by_value>`.
To access multiple variables at once one should use
:func:`DataBase.view*<tomate.data_base.DataBase.view>`. In which case it will
