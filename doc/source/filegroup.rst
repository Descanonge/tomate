...
In the process we also retrieve the in-file index for shared coordinates.
...
they are in a contiguous array.
...
When the user asks for a key, the key is reversed if the
